...
def sync_quickbooks_data(company_id: int):
    ...
    if not sync_manager.auth_client:
        ...
        sync_manager.db.log_sync(
            company_id, "quickbooks", "mock_seed", inv_count + bill_count, True
        )
        print(
            f"✅ Mock QuickBooks data seeded: {inv_count} invoices, {bill_count} bills"
        )
        return True
    ...
    try:
        ...
        if not qb_tokens:
            print(
                f"❌ No QuickBooks tokens found for company {company_id} — using mock data"
            )
            inv_count, bill_count = seed_mock_quickbooks(sync_manager.db, company_id)
            sync_manager.db.log_sync(
                company_id, "quickbooks", "mock_seed", inv_count + bill_count, True
        ...
        qb_client = QuickBooks(
            ...
            or st.secrets["QB_CLIENT_SECRET"],
        ...
        print(
            f"✅ QuickBooks sync completed: {invoices_synced} invoices, {bills_synced} bills"
        )
        return True
...
def setup_company_integrations():
    ...
    if PLAID_AVAILABLE and (
        os.getenv("PLAID_CLIENT_ID") or st.secrets.get("PLAID_CLIENT_ID")
    ):
