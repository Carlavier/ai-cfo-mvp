...
from data_sync_script import DataSyncManager
...
class QuickBooksClient:
    def __init__(self):
        ...
        self.client_secret = (
            os.getenv("QB_CLIENT_SECRET") or st.secrets["QB_CLIENT_SECRET"]
        )
...
class DeepSeekClient:
    ...
    def analyze_financial_data(self, query: str, context_data: Dict) -> str:
        ...
        if not self.api_key or self.api_key == "push_your_key_here":
            print("Missing api key! Add your api key")
        ...
        try:
            ...
            response = requests.post(
                self.base_url, headers=headers, json=payload, timeout=100
            )
...
I'll provide detailed analysis based on your actual financial data."""
    ...
    def analyze_financial_data_with_conversation(
        self, query: str, context_data: Dict, conversation: List[Dict] = None
    ) -> str:
...
class CashFlowAgent:
    ...
    def get_financial_summary(self, company_id: int) -> Dict:
        ...
        current_cash = sum(
            acc["current_balance"] for acc in accounts if acc["type"] in ["depository"]
        )
        ...
        # Calculate runway
        runway_days = (
            int(current_cash / (monthly_expenses / 30)) if monthly_expenses > 0 else 999
        )
    ...
    def generate_cash_flow_projection(self, company_id: int) -> List[Dict]:
        ...
        for week in range(14):  # 0-13 weeks
            ...
            income_variance = (
                weekly_income * random.uniform(0.8, 1.2) if week > 0 else 0
            )
    ...
    def get_ai_insights(
        self, company_id: int, company_name: str, user_question: str = None
    ) -> str:
        ...
        if not user_question:
            user_question = (
                "Provide a financial health assessment and key recommendations"
            )
    ...
    def get_ai_insights_with_conversation(
        self,
        company_id: int,
        company_name: str,
        ...
        if hasattr(self.ai, "analyze_financial_data_with_conversation"):
            return self.ai.analyze_financial_data_with_conversation(
                user_question, financial_data, conversation=conversation
            )
        else:
